...
export default function App() {
  const { state, actions } = useBrickPilotStore(brickPilotRepository.load);
  ...
  useEffect(() => {
    ...
    const tickRate = Math.max(120, 700 - state.preferences.speed * 50);
    const timer = window.setInterval(() => actions.tick(), tickRate);

    return () => window.clearInterval(timer);
  }, [actions, state.preferences.speed, state.status]);
